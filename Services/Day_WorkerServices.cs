...
namespace SchiftPlanner.Services
{
    public class Day_WorkerServices : IDay_WorkerServicesFirstGenerate, IDay_Worker_EditServices, IDay_Term
    {
        ...
        public async Task<List<Day_Worker_Timetable>> Get_Days_Worker(int Id_Timetable)
        {
            List<Day_Worker_Timetable> Days = _context.Day_Worker_Timetable.Where(s => s.Id_Timetable == Id_Timetable && s.Date > DateTime.Now).ToList();
            return Days;
        }
        ...
        {
            Day_Worker_Timetable day = _context.Day_Worker_Timetable.Where(s => s.Timetable_Day == Timetable_Day).FirstOrDefault();
            if (day.IsWork == true)
            {
                day.IsWork = false;
            }
        ...
        public async Task EditIsWorkRangeOfDays(int Id_Timetable, DateTime DayStart, DateTime DayEnd, bool IsWork)
        {
            ...
            List<Day_Worker_Timetable> day_Worker_Timetables = _context.Day_Worker_Timetable
            .Where(s => s.Id_Timetable == Id_Timetable && s.Date >= DayStart && s.Date <= DayEnd).ToList();
            foreach (Day_Worker_Timetable day in day_Worker_Timetables)
            {
                day.IsWork = IsWork;
        ...
        {
            List<Day_Worker_Timetable> day_Worker_Timetables = _context.Day_Worker_Timetable.Where(s => s.Id_Timetable == Id_Timetable && s.Date > DateTime.Now).ToList();
        ...
        public async Task<bool> IsTermChecked(string? Timetable_Day_User, DateTime dateTime, TimeSpan timeStart, TimeSpan timeEnd, int Id_Timetable)
        {
            if (Timetable_Day_User == null)
            {
                bool isAvailable = !_context.Day_Worker_Claimed.Any(dc =>
        ...
        {
            Day_Worker_Timetable day_Worker_ = _context.Day_Worker_Timetable.Find(Timetable_Day);
            if (TimeStart >= day_Worker_.TimeStart && TimeStart <= day_Worker_.TimeEnd && TimeEnd >= day_Worker_.TimeStart && TimeEnd <= day_Worker_.TimeEnd && TimeEnd > TimeStart)
            {
                return true;
            }
        ...
        {
            Worker_Timetable Worker_ = _context.Worker_Timetable.Where(s => s.Id_Timetable == Id_Timetable).First();
            bool isAvailable = await IsTermChecked(null, dateTime, TimeStart, TimeEnd, Id_Timetable);
            bool startendcheck = await StartEndCheck(Timetable_Day, TimeStart, TimeEnd);

            if (isAvailable && startendcheck)
            {
                var day_Worker = new Day_Worker_Claimed
                {
                    ...
                    Day_Worker_Timetable = _context.Day_Worker_Timetable.Where(s => s.Timetable_Day == Timetable_Day).FirstOrDefault()
                };

                _context.Day_Worker_Claimed.Add(day_Worker);
                _context.SaveChanges();
            }
        }
        ...
        {
            Worker_Timetable Worker_ = _context.Worker_Timetable.Where(s => s.Id_Timetable == Id_Timetable).First();
            bool isAvailable = await IsTermChecked(Timetable_Day_User, dateTime, TimeStart, TimeEnd, Id_Timetable);
            bool startendcheck = await StartEndCheck(Timetable_Day, TimeStart, TimeEnd);

            if (isAvailable && startendcheck)
            {
                Day_Worker_Claimed _claimed = _context.Day_Worker_Claimed.Find(Timetable_Day_User);
        ...
        {
            Day_Worker_Claimed day_Claimed = _context.Day_Worker_Claimed.Find(Timetable_Day_User);
            _context.Day_Worker_Claimed.RemoveRange(day_Claimed);
            _context.SaveChanges();
        }
        ...
        {
            List<Day_Worker_Timetable> days = _context.Day_Worker_Timetable.Where(s => s.Id_Timetable == Id_Timetable && s.Date >= DateTime.Now).ToList();
            foreach (Day_Worker_Timetable day in days)
            {
                day.TimeStart = start;
                day.TimeEnd = end;
            }
        ...
        public async Task<List<Day_Worker_Timetable>> GenerateFirstDay_Worker_Timetable(int Id_Timetable)
        {
            List<Day_Worker_Timetable> timetableList = new List<Day_Worker_Timetable>();
        ...
        public static Day_Worker_Timetable CreateTimetableDay(DateTime date, int Id_Timetable)
        {
            return new Day_Worker_Timetable
            {
